...
if __name__=='__main__':
    ...
    #write('hh_guitar.wav', samplerate, audio.astype(np.int16))
    ...
    #mixing
    mix_file = 'hh_mix.wav'
    guitar_len = len(audio)
    vocal_len = len(sound1.audio_data)
    max_len = max(vocal_len, guitar_len)
    
    mix = np.zeros(max_len)
    mix = np.append(sound1.audio_data, np.zeros(max_len - vocal_len +1)) + np.append(audio, np.zeros(max_len - guitar_len+1))
    write(mix_file, sample_freq, mix.astype(np.int16))
